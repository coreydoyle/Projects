...
{
	m_tank.reset(m_level.m_tank.m_position);
	m_aiTank.reset(m_level.m_aiTank.m_position);
}
...
void Game::update(double dt)
{
	...
	m_aiTank.collideWithPlayer(m_tank);
	if (m_aiTank.collideWithPlayer(m_tank) == true)
	{
	...
	case GameState::GAME_LOSE:
		m_timeSinceGameLost += dt;
		if (m_timeSinceGameLost > RESTART_DELAY)
		{
			restartGame();
			m_timeSinceGameLost = 0;
