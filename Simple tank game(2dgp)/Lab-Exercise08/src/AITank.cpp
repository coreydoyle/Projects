...
bool AITank::collideWithPlayer(Tank& playerTank) const
{
...
void AITank::reset(sf::Vector2f t_position)
{
	m_tankBase.setPosition(t_position);
	m_turret.setPosition(t_position);
	m_rotation = 0;
...
{
	m_health = m_health - t_damageAmount;
}

sf::Sprite AITank::getTankBase()
{
	return m_tankBase;
}

int AITank::getHealth()
{
	return m_health;
}
