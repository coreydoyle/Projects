...
////////////////////////////////////////////////////////////t_
void ProjectilePool::update(double t_dt, std::vector<sf::Sprite> & t_wallSprites, std::function<void(int)>& t_funcApplyDamage, sf::Sprite t_tankBase)
{	
	...
	for (int i = 0; i < s_POOL_SIZE; i++)
	{
		...
		if( !m_projectiles.at(i).update(t_dt, t_wallSprites, t_funcApplyDamage, t_tankBase))
		{
