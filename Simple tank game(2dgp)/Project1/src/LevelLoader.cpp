...
void operator >> (const YAML::Node& t_targetNode, TargetData& t_target)
{
	...
	t_target.m_scale = t_targetNode["scale"].as<float>();
}
...
void operator >> (const YAML::Node& t_levelNode, LevelData& t_level)
{
	...
	//It takes the position of the tank from the yaml file and assigns it to the m_tank
	//t_levelNode["tank"] >> t_level.m_tank;
	...
	for (unsigned i = 0; i < targetNode.size(); ++i)
	{
		TargetData target;
		targetNode[i] >> target;
		t_level.m_target.push_back(target);
	}
}
