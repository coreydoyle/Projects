...
void Game::init()
{
	...
	tank.setTextureRect(sf::IntRect(0, 95, 246, 114));
	for (TankData data : m_level.m_tank)
	{
		tank.setPosition(data.m_position);
		tank.setScale(sf::Vector2f(data.m_scale, data.m_scale));
		tank.setOrigin(123, 57);
		tank.setRotation(90);
		m_tankSprites.push_back(tank);
	}
	sf::Sprite turret;
	turret.setTexture(texture);
	turret.setTextureRect(sf::IntRect(247, 0, 191, 94));
	for (TankData data : m_level.m_tank)
	{
		turret.setPosition(data.m_position);
		turret.setScale(sf::Vector2f(data.m_scale, data.m_scale));
		turret.setOrigin(95.5f, 47);
		turret.setRotation(90);
		m_tankSprites.push_back(turret);
	}
...
void Game::render()
{
	...
	for (sf::Sprite& tank : m_tankSprites)
	{
		m_window.draw(tank);
	}
#ifdef TEST_FPS
